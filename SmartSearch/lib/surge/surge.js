...
//
//修改记录：
//2014-05-12: zzc    修改parse_filter方法，解决在IE7下使用以关键字命名的属性（如：default、class等）会报错的bug
//2014-04-25： zzc    修改compile函数，支持嵌套循环
//
...
(function (global) {
    ...
    surge.register = function (name, func) {
        if (/^[_a-zA-Z]\w*/.test(name)) {
            this.__builtins[name] = func;
            return this;
        }
    ...
    if (!String.prototype.hasOwnProperty('trim'))
        String.prototype.trim = function () {
            return this.replace(regex_trim, '');
        };
    ...
    function is_constant(a) {
        return regex_number.test(a[0]) || regex_const_string.test(a);
    }
    ...
    surge.register('trim',function (a) {
        ...
        return a;
            ...
            if (typeof a === 'string')
                return a.replace(/^([a-z])/, function (m) {
            ...
            return a.replace(/[<>'"]|&(?!amp;)/g, function (m) {
                return escapeMap[m];
            });
            ...
            if (typeof a === 'string')
                return a.replace(/\B(\w*)/g, function (m) {
                    return m.toLowerCase();
                })
                    .replace(/\b(\w)|\b(\w)/g, function (m) {
                        return m.toUpperCase();
                    });
            return a;
    ...
    surge.compile = function (source) {
        ...
        //add by zzc
        function isDefaultKey(s) {
            return '@default@class@'.indexOf('@' + s + '@') != -1;
        }
        ...
        function parse_filter(src) {
            ...
            if (fi > 0) {
                ...
                var fa = p2.split(':');
                var f = "_" + parse_prop(fa[0].trim());
                //var f = "_['" + fa[0].trim() + "']";//modify by zzc
                if (fa.length > 1)
                ...
                if (is_constant(src) || has_var(src.split('.')[0]))
                    ...
                    return "context" + parse_prop(src);
                    //return "context['" + src + "']";//modify by zzc
                }
            }
        }
                            ...
                            if (pos2 > -1) {
                                ...
                                if (word == 'if' || word == 'elif') {
                                    ...
                                    if (regex_for.test(token)) {
                                        ...
                                        /**************************************************************/
                                        //stmpt += 'for(var i=0,j=' + var_name + '.length;i<j;i++){var '+ RegExp.$1 + '='+var_name+'[i];';//by zhouzc 这是原来的，下面三行是我添加的
                                        var index_i = 'i' + var_index;
                                        var index_j = 'j' + var_index;
                                        stmpt += 'for(var ' + index_i + '=0,' + index_j + '=' + var_name + '.length;' + index_i + '<' + index_j + ';' + index_i + '++){var ' + RegExp.$1 + '=' + var_name + '[' + index_i + '];';
                                        /**************************************************************/
